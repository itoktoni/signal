...
class SniperService extends AnalysisService
{
    ...
    {
        // Get market data
        $klines = $this->getKlines($symbol, '1h', 100);
        if (!$klines) {
    ...
    private function calculateLevels(array $klines, string $signal): array
    {
        ...
        if ($signal === 'long') {
            ...
            // For hold positions - use safe defaults to prevent division by zero
    ...
    private function calculateConfidence(array $indicators, string $signal): float
    {
        ...
        if (($signal === 'long' && $indicators['ema9'] > $indicators['ema21']) ||
            ($signal === 'short' && $indicators['ema9'] < $indicators['ema21'])) {
            $confidence += 10;
        }
        ...
        if (($signal === 'long' && $indicators['price_change'] > 2) ||
            ($signal === 'short' && $indicators['price_change'] < -2)) {
            $confidence += 10;
        }
