...
class SimpleAnalysis implements AnalysisInterface
{
    ...
    public function analyze(string $symbol, float $amount = 100, string $timeframe = '1h', ?string $forcedApi = null): object
    {
        try {
            ...
            // Store the symbol and current price for later use
            $this->lastAnalyzedSymbol = $symbol;
            $this->lastCurrentPrice = $currentPrice;
     ...
     *
     * @param string|null $symbol Optional symbol to get current price for. If not provided, uses last analyzed symbol.
    ...
    public function getCurrentPrice(?string $symbol = null): float
    {
        try {
            ...
            if (!$symbolToUse) {
                Log::warning("SimpleAnalysis: No symbol available for getCurrentPrice");
                return 0.0;
            }

            // If we have a stored price and it's for the same symbol, return it
            if ($this->lastCurrentPrice && $symbolToUse === $this->lastAnalyzedSymbol) {
                return $this->lastCurrentPrice;
            }

            // Otherwise, fetch fresh price from API
            $currentPrice = $this->apiManager->getCurrentPrice($symbolToUse);

            // Update stored values
            $this->lastAnalyzedSymbol = $symbolToUse;
            $this->lastCurrentPrice = $currentPrice;

            return $currentPrice;
        } catch (\Exception $e) {
            Log::error("SimpleAnalysis: Failed to get current price", [
                'symbol' => $symbol,
                'error' => $e->getMessage()
            ]);
            return 0.0;
