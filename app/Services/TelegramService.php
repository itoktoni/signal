...
class TelegramService
{
    ...
    public function sendAnalysisResult(string $coinCode, object $analysis, float $currentPrice, $apiProvider): bool
    {
        ...
        $message .= "💰 <b>Current Price:</b> $" . number_format($analysis->price, 4) . "\n";
        $message .= "💰 <b>Rupiah Price:</b> Rp" . number_format($analysis->price * $rupiah, 0, ',', '.') . "\n\n";
        ...
        if (isset($analysis->notes) && !empty($analysis->notes)) {
            $notesText = is_array($analysis->notes) ? implode("\n", $analysis->notes) : $analysis->notes;
            $message .= "\n📝 <b>Notes:</b>\n{$notesText}\n";
        }
