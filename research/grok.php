...
$MIN_WINRATE        = floatval(getenv('MIN_WINRATE') ?: 0.9); // New: minimum winrate threshold
$BACKTEST_LIMIT     = intval(getenv('BACKTEST_LIMIT') ?: 500); // New: klines for backtest
$SCORE_THRESHOLD    = intval(getenv('SCORE_THRESHOLD') ?: 3); // New: score threshold for signal in backtest
$MIN_TRADES         = intval(getenv('MIN_TRADES') ?: 10); // New: min trades for valid winrate
...
function getTrendingFromCoingecko($limit=15) {
    ...
    foreach ($res['coins'] as $c) {
        ...
        $change = null;
        if (isset($it['data']['price_change_percentage_24h_in_currency'])) {
            $pc = $it['data']['price_change_percentage_24h_in_currency'];
...
function computeIndicators(array $klines) {
    if (count($klines) < 30) return null;
...
// -------------------- backtest winrate --------------------
function calculateWinRate($pair, $interval='1h') {
    global $BACKTEST_LIMIT, $SCORE_THRESHOLD, $MIN_TRADES, $REQUEST_DELAY_MS;
    $klines = getBinanceKlines($pair, $interval, $BACKTEST_LIMIT);
    if (!$klines || count($klines) < $BACKTEST_LIMIT) return 0.0;

    $wins = 0;
    $total_trades = 0;
    $min_history = 200; // Enough for indicators
    $future_buffer = 20; // Candles to check forward

    for ($i = $min_history; $i < $BACKTEST_LIMIT - $future_buffer; $i++) {
        $past_klines = array_slice($klines, 0, $i + 1); // Include "current" candle
        ...
        if ($analysis['score'] >= $SCORE_THRESHOLD) {
            ...
            if ($hit_tp || $hit_sl) {
                ...
                // If neither hit, count as loss to be conservative
                $total_trades++;
            }
        }
    }
    ...
    return ($total_trades >= $MIN_TRADES) ? ($wins / $total_trades) : 0.0;
}
...
function analyzeMultiTFForPair($pair, $trendData=null, $winrate=0.0) {
    global $USD_TO_IDR;
    ...
    elseif (isset($validScores['H1']) && $validScores['H1']>0 && (isset($validScores['H4']) && $validScores['H4']<=0 || isset($validScores['1D']) && $validScores['1D']<=0)) $overall = "⚠️ H1 oke — hanya scalping (jangan hold lama)";
    ...
    $out[] = "📈 Winrate historis (backtest {$BACKTEST_LIMIT} klines 1h): " . round($winrate * 100, 1) . "%";
    $out[] = "";
...
$btcBlock = analyzeMultiTFForPair('BTCUSDT', $btcTrend, $btcWinrate);
$header = "📊 *BTC Barometer*\n\n" . $btcBlock . "\n\n";
echo $header . "\n";

// 5) analyze each candidate and build message if winrate >= MIN_WINRATE
$blocks = [];
foreach ($candidates as $pair) {
    sleep_ms($REQUEST_DELAY_MS);
    $winrate = calculateWinRate($pair);
    if ($winrate < $MIN_WINRATE) {
        echo "Skipping $pair - winrate " . round($winrate * 100, 1) . "% < $MIN_WINRATE\n";
        continue;
    }
...
if (count($blocks) > 0) {
    ...
    $finalMsg .= "Tidak ada coin trending dengan winrate >= " . ($MIN_WINRATE * 100) . "% saat ini.";
}
